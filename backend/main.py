...
def seed_data(db: Session) -> None:
    ...
    for item in SEED_ANIMALS:
        ...
        if existing:
            ...
            # IMPORTANT: do NOT overwrite progress on startup.
            # Progress must be changed only by donations.
        else:
            db.add(Animal(**item))
...
def donate(payload: DonationCreate):
    ...
    try:
        ...
        # Update progress based on a fixed internal goal (100k). We don't expose goal/amounts in UI.
        goal_amount = 100_000
        increment = int(round((payload.amount / goal_amount) * 100))
        current_progress = int(getattr(animal, "progress", 0) or 0)
        setattr(animal, "progress", min(100, max(0, current_progress + increment)))
